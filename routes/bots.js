...
const perm = (level) =>
	async (req, res, next) => {
		...
		if (!getBot) {
			...
			res.status(400).render('error', { title: 'Error', status: 401, message: 'You are not allowed to edit other\'s bots' });
		}
};
...
const clean = (html) => {
  ...
  $('object').remove();
  $('blockquote').remove();
  return $.html().replace(/(href|src)=('|"|`)javascript:.*('|"|`)/,"");
};

router.get('/add', user.configure, user.auth, user.inServer, user.isBanned, (req, res, next) => {
  res.render('bots/add', { title: 'Add Bot' })
});

router.post('/add', user.configure, user.auth, user.inServer, user.isBanned, async (req, res, next) => {
  let checkBot = await r.table('bots').get(req.body.client_id);
  if (checkBot) return res.status(409).render('error', { title: 'Error', status: 409, message: 'This bot has already been added to the list.' })
  let invite;
  if (req.body.invite == '') {
    invite = `https://discordapp.com/oauth2/authorize?client_id=${req.body.client_id}&scope=bot`
  } else {
    if (typeof req.body.invite !== 'string') {
			return res.status(400).render('error', { title: 'Error', status: 400, message: 'You provided an invalid invite.' })
		} else if (req.body.invite.length > 2000) {
      return res.status(413).render('error', { title: 'Error', status: 413, message: 'You provided an invite that was too long (2000)' })
  ...
  if (req.body.client_id.length > 18) {
      return res.status(400).render('error', { title: 'Error', status: 400, message: 'The bot id must not be greater than 18 numbers.' })
    ...
    return res.status(400).render('error', { title: 'Error', status: 400, message: 'You can\'t have more than 5 owners.'}) 
      ...
      return res.status(401).render('error', { title: 'Error', status: 401, message: 'You cannot add users to the list.'})
    ...
    if (req.body.owners == '') {
      ...
      r.table('bots').insert({ 
          ...
          invite: invite,
          short_desc: req.body.short_desc.replace(/<(script|object|blockquote)[\s\S]*?>[\s\S]*?<\/(script|object|blockquote)>/,"").replace(/(href|src)=('|"|`)javascript:.*('|"|`)/,"")
,
          long_desc: req.body.long_desc.replace(/<(script|object|blockquote)[\s\S]*?>[\s\S]*?<\/(script|object|blockquote)>/,"").replace(/(href|src)=('|"|`)javascript:.*('|"|`)/,"")
,
    ...
    client.channels.get(settings.channels.weblog).send("\:inbox_tray: <@" + req.user.id + "> added \`" + discordResponse.username + "\` <@" + req.body.client_id + "> (\:eyes: <@&555455030136668160>)\n<http://216f0b01.ngrok.io/bots/" + req.body.client_id + ">")
...
router.post('/verification', user.configure, user.auth, user.inServer, async (req, res, next) => {
  ...
  if(!checkBot.approved) return res.status(401).render('error', { title: 'Error', status: 401, message: 'You cannot apply for verification until your bot is approved, and has been on the list for at least 3 days. Read <a href="https://e5e2b250.ngrok.io/docs/verification"> https://e5e2b250.ngrok.io/docs/verification</a>. '});
  ...
  try {
    await client.users.get(req.user.id).send("<:" + settings.emojis.verified + "> Your verification application for \`" + checkBot.name + "\` <@" + req.body.bot + "> has been sent! Please wait while a Website Administrator reviews it.")
  } catch(e) {
  ...
  client.channels.get(settings.channels.weblog).send("<a:" + settings.emojis.verified + "> <@" + req.user.id + "> applied \`" + checkBot.name + "\` <@" + req.body.bot + "> for verification.\n< https://e5e2b250.ngrok.io/bots/" + req.body.bot + ">");
...
router.post('/:id/delete', user.configure, user.auth, perm(1), (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    r.table('bots_backup').insert({
      ...
      library: bot.library,
      invite: bot.invite,
      short_desc: bot.short_desc.replace(/<(script|object|blockquote)[\s\S]*?>[\s\S]*?<\/(script|object|blockquote)>/,"").replace(/(href|src)=('|"|`)javascript:.*('|"|`)/,"")
,
      long_desc: bot.long_desc.replace(/<(script|object|blockquote)[\s\S]*?>[\s\S]*?<\/(script|object|blockquote)>/,"").replace(/(href|src)=('|"|`)javascript:.*('|"|`)/,"")
,
...
router.post('/:id/edit', user.configure, user.auth, perm(1), async (req, res, next) => {
  ...
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    if (typeof req.body.invite !== 'string') {
      ...
      return res.status(413).render('error', { title: 'Error', status: 413, message: 'You provided an invite that was too long (2000 Max)' })
		} else if (!/^https?:\/\//.test(req.body.invite)) {
      return res.status(400).render('error', { title: 'Error', status: 400, message: 'Your invite must use HTTP or HTTPS' });
  } else if (!req.body.invite.startsWith('https://discordapp.com/') ){
    return res.status(400).render('error', { title: 'Error', status: 400, message: 'Invalid invite URL. It must start with: https://discordapp.com/.' });
...
,
  ...
  if (update.unchanged) {
    ...
    client.channels.get(settings.channels.weblog).send("\:pencil: " + req.user.username + "#" + req.user.discriminator + " (" + req.user.id + ") edited \`" + discordResponse.username + "\` <@" + req.params.id + "> by " + botDevs + "\n<https://e5e2b250.ngrok.io/bots/" + req.params.id + ">")
    res.status(200).render('error', { title: 'Success', status: 200, message: 'Successfully updated bot.'});
  }
}) }) }) 
});

router.get('/resubmit/:id', user.configure, user.auth, user.inServer, user.isBanned, (req, res, next) => {
  r.table('bots_backup').get(req.params.id).run(async (error, bot) => {
    if (!bot) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot cannot be found.'});
    if (bot.owner != req.user.id) return res.status(401).render('error', { title: 'Error', status: 401, message: 'You are not the owner of that bot.' }); 
...
router.get('/resubmit/:id/delete', user.configure, user.auth, (req, res, next) => {
  r.table('bots_backup').get(req.params.id).run(async (error, bot) => {
    ...
    res.status(200).render('error', { title: 'Success', status: 200, message: 'Successfully removed bot data.' })
  })
});


router.post('/resubmit/:id', user.configure, user.auth, user.inServer, user.isBanned, async (req, res, next) => {
  r.table('bots_backup').get(req.params.id).run(async (error, bot) => {
    if (!bot) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot cannot be found.'});
    if (bot.owner != req.user.id) return res.status(401).render('error', { title: 'Error', status: 401, message: 'You are not the owner of that bot.' }); 
...
router.get('/:id/approve', user.configure, user.auth, user.mods, (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
...
router.get('/:id/verify', user.configure, user.auth, user.admins, (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    if (!bot) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot does not exist.'});
    if (bot.approved == false) return res.status(400).render('error', { title: 'Error', status: 400, message: 'That bot is not approved yet.'});
    ...
    try {
      await client.users.get(bot.owner).send("<:" + settings.emojis.verified + "> Your bot \`" + bot.name + "\` <@" + bot.id + "> has been verified by <@" + req.user.id + ">! Enjoy your perks! \:tada:")
    } catch(e) {
      console.log("Failed to alert bot dev.")
    }
...
router.get('/:id/unverify', user.configure, user.auth, user.admins, (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    if (!bot) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot does not exist.'});
    if (bot.approved == false) return res.status(400).render('error', { title: 'Error', status: 400, message: 'That bot is not approved yet.'});
...
router.post('/:id/unverify', user.configure, user.auth, user.admins, (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    if (!bot) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot does not exist.'});
    if (bot.approved == false) return res.status(400).render('error', { title: 'Error', status: 400, message: 'That bot is not approved yet.'});
    let owners = bot.owners;
    ...
    if (bot.verified == true) {
      try {
        await client.users.get(bot.owner).send("<a:" + settings.emojis.verified + "> Your bot \`" + bot.name + "\` <@" + bot.id + "> has been unverified by <@" + req.user.id + ">! You have lost your perks. \:wave:\n\n\:page_facing_up: Reason: \`" + req.body.reason + "\`")
      } catch(e) {
        console.log("Failed to alert bot dev.")
      }
      ...
      client.channels.get(settings.channels.weblog).send("<:" + settings.emojis.verified + "> <@" + req.user.id + "> denied the verification of \`" + bot.name + "\` <@" + bot.id + ">");
...
router.get('/:id/remove', user.configure, user.auth, user.mods, (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    if (!bot) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot does not exist.'});
...
router.post('/:id/remove', user.configure, user.auth, user.mods, (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    if (!bot) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot does not exist.'});
...
,
    ...
    if (bot.approved == true) {
      ...
      try {
        await client.users.get(bot.owner).send("\:wastebasket: Your bot \`" + bot.name + "\` <@" + bot.id + "> has been removed by <@" + req.user.id + ">\n\:page_facing_up: Reason: \`" + req.body.extra + "\`")
      } catch(e) {
...
router.get('/:id', user.configure, async (req, res, next) => {
  if (!await r.table('bots').get(req.params.id)) return res.status(404).render('error', { title: 'Error', status: 404, message: 'Bot does not exist.' });
  ...
  let desc = '';
  desc = clean(marked(bot.long_desc));
