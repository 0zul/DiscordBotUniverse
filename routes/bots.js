...
router.post('/verification', user.configure, user.auth, user.inServer, async (req, res, next) => {
  ...
  if(!checkBot.approved) return res.status(401).render('error', { title: 'Error', status: 401, message: 'You cannot apply for verification until your bot is approved, and has been on the list for at least 3 days. Read <a href="https://3b2cde4c.ngrok.io/docs/verification">https://3b2cde4c.ngrok.io/docs/verification</a>. '});
...
router.post('/:id/unverify', user.configure, user.auth, user.admins, (req, res, next) => {
  r.table('bots').get(req.params.id).run(async (error, bot) => {
    ...
    if (bot.verified == true) {
      ...
      try {
        await client.users.get(bot.owner).send("<a:" + settings.emojis.verified + "> Your bot \`" + bot.name + "\` <@" + bot.id + "> has been unverified by <@" + req.user.id + ">.\n\:page_facing_up: Reason: \`" + req.body.reason + "\`")
      } catch(e) {
        console.log("Failed to alert bot dev.")
      }
      client.channels.get(settings.channels.weblog).send("<a:" + settings.emojis.verified + "> <@" + req.user.id + "> denied the verification of \`" + bot.name + "\` <@" + bot.id + ">");
